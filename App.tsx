...
const App = () => {
  ...
  async function callSSLPinning() {
    try {
      console.log('🔒 Starting SSL-pinned request…');
      const response = await fetch('https://192.168.1.39:3000/', {
        method: 'GET',
        sslPinning: {certs: ['server_cert']},
        timeoutInterval: 15000,
        headers: {Accept: 'application/json'},
      });

      console.log(`⚙️  Raw response object:`, response);
      ...
      try {
        ...
        console.warn('⚠️  Response wasn’t valid JSON:', jsonErr);
        return;
      }
  ...
  return (
    <View style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
      <Text style={{color: '#000'}}>SSL Pinning Example</Text>
    </View>
  );
};
export default App;
